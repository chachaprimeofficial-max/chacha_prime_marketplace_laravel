...
class StorefrontController extends Controller {
 ...
 public function product(Request $request,int $id){$product=Product::with(['vendor','category','images'])->where('status','published')->findOrFail($id);$customerType=$request->user()?->role==='b2b_customer'?'b2b':'b2c';$tiers=$product->pricingTiers()->where('customer_type',$customerType)->orderBy('min_quantity')->get();$price=app(PricingService::class)->unitPrice($product,1,$customerType);return view('storefront.product',compact('product','tiers','price','customerType'));}
 ...
 public function placeOrder(Request $request){
  ...
  $orderIds=DB::transaction(function()use($products,$cart,$address,$data,$request,$pricing,$customerType,$coupon){
   ...
   $globalRemaining=$coupon && is_null($coupon->vendor_id) ? (float)$coupon->value : 0;
   foreach($vendorGroups as $vendorId=>$items){
    $subtotal=$groupSubtotals[(int)$vendorId];$discount=0;$couponApplies=false;
    if($coupon && (is_null($coupon->vendor_id) || (int)$coupon->vendor_id===(int)$vendorId)){
     $couponApplies=$coupon->min_order===null || $subtotal >= (float)$coupon->min_order;
     if($couponApplies){
      if($coupon->type==='percent'){$discount=$subtotal*((float)$coupon->value/100);}else{$discount=is_null($coupon->vendor_id)?min($globalRemaining,$subtotal):(float)$coupon->value;}
      if($coupon->max_discount!==null)$discount=min($discount,(float)$coupon->max_discount);
      $discount=min($discount,$subtotal);
      if($discount>0){$appliedCoupon=true;if($coupon->type==='fixed' && is_null($coupon->vendor_id))$globalRemaining=max(0,$globalRemaining-$discount);}
     }
    }
 ...
 public function liveShopping(){ $streams=DB::table('live_streams')->join('vendors','vendors.id','=','live_streams.vendor_id')->whereIn('live_streams.status',['scheduled','live'])->select('live_streams.*','vendors.business_name')->orderBy('live_streams.starts_at')->paginate(20);foreach($streams as $s){$s->youtube_video_id=$s->youtube_video_id ?: $this->youtubeId($s->youtube_url);}return view('storefront.live-shopping',compact('streams'));}
