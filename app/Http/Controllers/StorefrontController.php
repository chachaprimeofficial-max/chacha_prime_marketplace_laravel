...
use App\Services\MarketplaceCostService;
...
class StorefrontController extends Controller {
 ...
 public function shop(Request $request){$countryId=(int)session('marketplace_country_id',0);$q=trim((string)$request->get('q',''));$category=$request->integer('category');$minPrice=$request->input('min_price');$maxPrice=$request->input('max_price');$inStock=$request->boolean('in_stock');$sort=$request->get('sort','relevance');$products=Product::with(['category','images','vendor'])->where('status','published')->when($countryId,fn($x)=>$x->where(fn($z)=>$z->where('country_id',$countryId)->orWhereHas('marketplaces',fn($m)=>$m->where('countries.id',$countryId)->where('product_marketplaces.active',1))))->when($q,fn($x)=>$x->where(fn($y)=>$y->where('name','like',"%{$q}%")->orWhere('description','like',"%{$q}%")->orWhere('sku','like',"%{$q}%")))->when($category,fn($x)=>$x->where('category_id',$category))->when($minPrice!==null&&$minPrice!=='',function($x)use($minPrice,$countryId){$x->where(function($q)use($minPrice,$countryId){$q->where(function($z)use($minPrice,$countryId){$z->where('products.country_id',$countryId)->where('products.retail_price','>=',(float)$minPrice);})->orWhereHas('marketplaces',fn($m)=>$m->where('countries.id',$countryId)->where('product_marketplaces.active',1)->where('product_marketplaces.retail_price','>=',(float)$minPrice));});})->when($maxPrice!==null&&$maxPrice!=='',function($x)use($maxPrice,$countryId){$x->where(function($q)use($maxPrice,$countryId){$q->where(function($z)use($maxPrice,$countryId){$z->where('products.country_id',$countryId)->where('products.retail_price','<=',(float)$maxPrice);})->orWhereHas('marketplaces',fn($m)=>$m->where('countries.id',$countryId)->where('product_marketplaces.active',1)->where('product_marketplaces.retail_price','<=',(float)$maxPrice));});})->when($inStock,function($x)use($countryId){$x->where(function($q)use($countryId){$q->where(function($z)use($countryId){$z->where('products.country_id',$countryId)->where('products.stock','>',0)->where('products.stock_status','<>','out_of_stock');})->orWhereHas('marketplaces',fn($m)=>$m->where('countries.id',$countryId)->where('product_marketplaces.active',1)->where('product_marketplaces.stock','>',0));});})->when(in_array($sort,['price_asc','price_desc']),function($x)use($countryId,$sort){$direction=$sort==='price_asc'?'asc':'desc';$x->orderByRaw("(CASE WHEN products.country_id = ? THEN products.retail_price ELSE COALESCE((SELECT pm.retail_price FROM product_marketplaces pm WHERE pm.product_id = products.id AND pm.country_id = ? AND pm.active = 1 LIMIT 1), products.retail_price) END) {$direction}",[$countryId,$countryId]);})->when($sort==='newest',fn($x)=>$x->latest())->when($sort==='relevance',fn($x)=>$x->latest())->paginate(24)->withQueryString();$pricing=app(PricingService::class);foreach($products as $p){$p->setAttribute('marketplace_price',$pricing->unitPrice($p,1,'b2c',$countryId));$p->setAttribute('currency',$pricing->currency($p,$countryId));$p->setAttribute('marketplace_stock',$pricing->stock($p,$countryId));}$categories=DB::table('categories')->where('status',1)->whereNull('parent_id')->orderBy('sort_order')->orderBy('name')->get();return view('storefront.shop',compact('products','q','category','categories','minPrice','maxPrice','inStock','sort','countryId'));}
 public function search(Request $request){return $this->shop($request);}
 public function product(Request $request,int $id){$countryId=(int)session('marketplace_country_id',0);$product=Product::with(['vendor','category','brand','images','videos','variants'])->where('status','published')->when($countryId,fn($x)=>$x->where(fn($z)=>$z->where('country_id',$countryId)->orWhereHas('marketplaces',fn($m)=>$m->where('countries.id',$countryId)->where('product_marketplaces.active',1))))->findOrFail($id);$customerType=$request->user()?->role==='b2b_customer'?'b2b':'b2c';$tiers=$product->pricingTiers()->where('customer_type',$customerType)->orderBy('min_quantity')->get();$countryId=(int)(session('marketplace_country_id') ?: DB::table('countries')->where('code','PK')->value('id'));$pricing=app(PricingService::class);$price=$pricing->unitPrice($product,1,$customerType,$countryId);$currency=$pricing->currency($product,$countryId);$product->setAttribute('currency',$currency);$product->setAttribute('marketplace_stock',$pricing->stock($product,$countryId));$reviews=DB::table('reviews')->join('users','users.id','=','reviews.user_id')->where('reviews.product_id',$product->id)->where('reviews.status','approved')->select('reviews.*','users.name as user_name')->latest('reviews.id')->limit(10)->get();$reviewCount=DB::table('reviews')->where('product_id',$product->id)->where('status','approved')->count();$avgRating=(float)(DB::table('reviews')->where('product_id',$product->id)->where('status','approved')->avg('rating')??0);$buyers=(int)DB::table('order_items')->where('product_id',$product->id)->distinct('order_id')->count('order_id');$monthlyBought=(int)DB::table('order_items')->join('orders','orders.id','=','order_items.order_id')->where('order_items.product_id',$product->id)->where('orders.created_at','>=',now()->subDays(30))->sum('order_items.quantity');$brandProducts=Product::with(['images','brand'])->where('status','published')->where('id','<>',$product->id)->when($product->brand_id,fn($q)=>$q->where('brand_id',$product->brand_id))->latest()->limit(4)->get();$relatedProducts=Product::with(['images','brand'])->where('status','published')->where('id','<>',$product->id)->when($product->category_id,fn($q)=>$q->where('category_id',$product->category_id))->latest()->limit(4)->get();$shippingInfo=app(\App\Services\MarketplaceCostService::class)->shippingForVendor([$product],[$product->id=>1],$countryId,(int)$product->vendor_id,(float)$price);$countryName=DB::table('countries')->where('id',$countryId)->value('name')??'Selected marketplace';return view('storefront.product',compact('product','tiers','price','currency','customerType','reviews','reviewCount','avgRating','buyers','monthlyBought','brandProducts','relatedProducts','shippingInfo','countryName'));}
 public function cart(Request $request){$cart=$request->session()->get('cart',[]);$products=Product::where('status','published')->whereIn('id',array_keys($cart))->get();$customerType=$request->user()?->role==='b2b_customer'?'b2b':'b2c';$countryId=(int)(session('marketplace_country_id') ?: DB::table('countries')->where('code','PK')->value('id'));$pricing=app(PricingService::class);$prices=[];$currencies=[];$total=0;foreach($products as $p){$qty=(int)$cart[$p->id];$prices[$p->id]=$pricing->unitPrice($p,$qty,$customerType,$countryId);$currencies[$p->id]=$pricing->currency($p,$countryId);$p->setAttribute('currency',$currencies[$p->id]);$p->setAttribute('marketplace_stock',$pricing->stock($p,$countryId));$total+=$prices[$p->id]*$qty;}return view('storefront.cart',compact('products','cart','total','prices','currencies','countryId'));}
 public function addToCart(Request $request,int $id){$p=Product::where('status','published')->findOrFail($id);$qty=max(1,(int)$request->input('quantity',1));$countryId=(int)($request->session()->get('marketplace_country_id') ?: DB::table('countries')->where('code','PK')->value('id'));$available=app(PricingService::class)->stock($p,$countryId);$cart=$request->session()->get('cart',[]);$newQty=($cart[$id]??0)+$qty;abort_if($available<$newQty,422,'Requested quantity exceeds available stock.');$cart[$id]=$newQty;$request->session()->put('cart',$cart);return back()->with('success',$p->name.' added to cart.');}
 ...
 public function checkout(Request $request){$cart=$request->session()->get('cart',[]);$products=Product::where('status','published')->whereIn('id',array_keys($cart))->get();abort_if($products->isEmpty(),404,'Cart is empty');$customerType=$request->user()->role==='b2b_customer'?'b2b':'b2c';$countryId=(int)(session('marketplace_country_id') ?: DB::table('countries')->where('code','PK')->value('id'));$pricing=app(PricingService::class);$costs=app(MarketplaceCostService::class);$prices=[];$currencies=[];$total=0;$shippingTotal=0;$taxTotal=0;$vendorShipping=[];foreach($products as $p){$qty=(int)$cart[$p->id];$prices[$p->id]=$pricing->unitPrice($p,$qty,$customerType,$countryId);$currencies[$p->id]=$pricing->currency($p,$countryId);$p->setAttribute('currency',$currencies[$p->id]);$p->setAttribute('marketplace_stock',$pricing->stock($p,$countryId));$total+=round($prices[$p->id]*$qty,2);}$groups=$products->groupBy('vendor_id');foreach($groups as $vendorId=>$items){$sub=0;foreach($items as $p){$sub+=round($prices[$p->id]*$cart[$p->id],2);}$ship=$costs->shippingForVendor($items,$cart,$countryId,(int)$vendorId,$sub);$tax=$costs->taxForVendor($items,$cart,$countryId,(int)$vendorId,$sub);$vendorShipping[$vendorId]=['subtotal'=>$sub,'shipping'=>$ship,'tax'=>$tax];$shippingTotal+=($ship['amount']??0);$taxTotal+=$tax;}$methods=app(PaymentService::class)->enabledMethods();$addresses=DB::table('addresses')->where('user_id',$request->user()->id)->orderByDesc('is_default')->get();$coupons=DB::table('coupons')->where('status',1)->where(fn($q)=>$q->whereNull('starts_at')->orWhere('starts_at','<=',now()))->where(fn($q)=>$q->whereNull('ends_at')->orWhere('ends_at','>=',now()))->where(fn($q)=>$q->whereNull('usage_limit')->orWhereColumn('used_count','<','usage_limit'))->orderBy('code')->get();return view('storefront.checkout',compact('products','cart','total','shippingTotal','taxTotal','vendorShipping','methods','addresses','prices','coupons','currencies','countryId'));}
 public function placeOrder(Request $request){
  ...
  $marketCountry=DB::table('countries')->where('id',$countryId)->first();
  abort_unless($marketCountry,422,'Marketplace country is unavailable.');
  abort_unless(strtoupper((string)$address->country)===strtoupper((string)$marketCountry->name) || strtoupper((string)$address->country)===strtoupper((string)$marketCountry->code),422,'Shipping address country must match the selected marketplace country.');
  ...
  $orderIds=DB::transaction(function()use($cart,$address,$data,$request,$pricing,$customerType,$couponCode){
   ...
   $countryId=(int)($request->session()->get('marketplace_country_id') ?: DB::table('countries')->where('code','PK')->value('id'));
   $costs=app(MarketplaceCostService::class);
   ...
   $currencies=$products->map(fn($p)=>$pricing->currency($p,$countryId))->map(fn($v)=>strtoupper((string)$v))->unique()->values();
   ...
   foreach($vendorGroups as $vendorId=>$items){
    $subtotal=0; foreach($items as $p){$qty=max(1,(int)$cart[$p->id]);$subtotal+=round($pricing->unitPrice($p,$qty,$customerType,$countryId)*$qty,2);}
    $discount=0;
    ...
    $shipping=$costs->shippingForVendor($items,$cart,$countryId,(int)$vendorId,$subtotal);
    $tax=$costs->taxForVendor($items,$cart,$countryId,(int)$vendorId,max(0,$subtotal-$discount));
    $order=new Order;
    $order->user_id=$request->user()->id;$order->country_id=$countryId;$order->order_number='CP-'.strtoupper(bin2hex(random_bytes(5)));$order->status='pending';$order->payment_status='pending';$order->fulfillment_status='unfulfilled';$order->currency=$currency;$order->subtotal=round($subtotal,2);$order->discount_total=$discount;$order->shipping_total=(float)($shipping['amount']??0);$order->tax_total=$tax;$order->grand_total=round(max(0,$subtotal-$discount+$order->shipping_total+$order->tax_total),2);$order->shipping_address=(array)$address;$order->billing_address=(array)$address;$order->save();
    foreach($items as $p){$qty=max(1,(int)$cart[$p->id]);$unit=round($pricing->unitPrice($p,$qty,$customerType,$countryId),2);DB::table('order_items')->insert(['order_id'=>$order->id,'vendor_id'=>$p->vendor_id,'product_id'=>$p->id,'product_name'=>$p->name,'sku'=>$p->sku,'quantity'=>$qty,'unit_price'=>$unit,'subtotal'=>round($unit*$qty,2),'vendor_status'=>'pending']);$market=DB::table('product_marketplaces')->where('product_id',$p->id)->where('country_id',$countryId)->where('active',1)->lockForUpdate()->first();if($market){$newStock=(float)$market->stock-$qty;abort_if($newStock<0,422,'Stock changed. Please review your cart.');DB::table('product_marketplaces')->where('id',$market->id)->update(['stock'=>$newStock]);}else{$newStock=(float)$p->stock-$qty;abort_if($newStock<0,422,'Stock changed. Please review your cart.');DB::table('products')->where('id',$p->id)->update(['stock'=>$newStock,'stock_status'=>$newStock<=0?'out_of_stock':'in_stock']);}}
 ...
 public function customerDashboard(Request $request){$user=$request->user();$stats=['orders'=>DB::table('orders')->where('user_id',$user->id)->count(),'pending'=>DB::table('orders')->where('user_id',$user->id)->whereIn('status',['pending','processing'])->count(),'wishlist'=>DB::table('wishlists')->where('user_id',$user->id)->count(),'notifications'=>DB::table('notifications')->where('user_id',$user->id)->whereNull('read_at')->count()];$recentOrders=DB::table('orders')->where('user_id',$user->id)->latest()->limit(5)->get();$addresses=DB::table('addresses')->where('user_id',$user->id)->orderByDesc('is_default')->limit(2)->get();return view('customer.dashboard',compact('user','stats','recentOrders','addresses'));}
 ...
 public function wishlist(Request $request){$countryId=(int)($request->session()->get('marketplace_country_id') ?: DB::table('countries')->where('code','PK')->value('id'));$items=DB::table('wishlists')->join('products','products.id','=','wishlists.product_id')->where('wishlists.user_id',$request->user()->id)->where('products.status','published')->select('wishlists.*','products.name','products.retail_price','products.currency','products.stock','products.country_id')->latest('wishlists.created_at')->get();$pricing=app(PricingService::class);foreach($items as $item){$product=Product::find($item->product_id);$item->marketplace_price=$pricing->unitPrice($product,1,'b2c',$countryId);$item->marketplace_currency=$pricing->currency($product,$countryId);$item->marketplace_stock=$pricing->stock($product,$countryId);$item->country_id=$countryId;}return view('customer.wishlist',compact('items','countryId'));}
