...
class VendorController extends Controller
{
 ...
 public function storeProduct(Request $request){
  ...
  $product=Product::create($d);$this->saveProductImages($request,$product,$images);$this->saveProductVideos($product,$videos);$this->saveProductVariants($product,$request->input('variations'));app(AuditLogService::class)->log('vendor.product.created','Product',$product->id,['vendor_id'=>$vendor->id,'name'=>$product->name]);return back()->with('success','Product submitted for admin approval.');
 }
 ...
 public function updateProduct(Request $request,int $id){
  ...
  $images=$request->file('images',[]);$videos=$request->file('videos',[]);unset($d['images'],$d['videos']);$d['stock_status']=$d['stock']>0?'in_stock':'out_of_stock';$before=$p->only(['name','retail_price','stock','status']);$p->update($d);$this->saveProductImages($request,$p,$images);$this->saveProductVideos($p,$videos);$this->saveProductVariants($p,$variations);app(AuditLogService::class)->log('vendor.product.updated','Product',$p->id,['before'=>$before,'after'=>$d]);return back()->with('success','Product updated.');
 }
