...
class VendorController extends Controller
{
 ...
 public function updateProduct(Request $request,int $id){
  ...
  $images=$request->file('images',[]);$videos=$request->file('videos',[]);$variations=$request->input('variations');unset($d['images'],$d['videos'],$d['variations']);$d['stock_status']=$d['stock']>0?'in_stock':'out_of_stock';$before=$p->only(['name','retail_price','stock','status']);$p->update($d);$this->saveProductImages($request,$p,$images);$this->saveProductVideos($p,$videos);$this->saveProductVariants($p,$variations);app(AuditLogService::class)->log('vendor.product.updated','Product',$p->id,['before'=>$before,'after'=>$d]);return back()->with('success','Product updated.');
 }
