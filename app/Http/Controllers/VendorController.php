...
class VendorController extends Controller
{
 ...
 public function storeProduct(Request $request){
  ...
  $product=Product::create($d);$this->ensureIdentifier($product);$this->saveProductImages($request,$product,$images);$this->saveProductVideos($product,$videos);$this->saveProductVariants($product,$request->input('variations'));app(AuditLogService::class)->log('vendor.product.created','Product',$product->id,['vendor_id'=>$vendor->id,'name'=>$product->name]);return back()->with('success','Product submitted for admin approval.');
 }
