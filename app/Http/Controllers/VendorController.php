...
class VendorController extends Controller
{
 ...
 public function updateProduct(Request $request,int $id){
  ...
  $images=$request->file('images',[]);$videos=$request->file('videos',[]);$variations=$request->input('variations');unset($d['images'],$d['videos'],$d['variations']);$d['stock_status']=$d['stock']>0?'in_stock':'out_of_stock';$before=$p->only(['name','retail_price','stock','status']);$p->update($d);$this->syncProductMarketplaces($p,(array)$request->input('marketplace_country_ids',[]));$this->ensureIdentifier($p);$this->saveProductImages($request,$p,$images);$this->saveProductVideos($p,$videos);$this->saveProductVariants($p,$variations);app(AuditLogService::class)->log('vendor.product.updated','Product',$p->id,['before'=>$before,'after'=>$d]);return back()->with('success','Product updated.');
 }

 private function syncProductMarketplaces(Product $product,array $countryIds): void {
  $ids=collect($countryIds)->map(fn($v)=>(int)$v)->filter()->unique()->reject(fn($id)=>(int)$id===(int)$product->country_id)->values();
  DB::table('product_marketplaces')->where('product_id',$product->id)->delete();
  foreach($ids as $countryId){DB::table('product_marketplaces')->insert(['product_id'=>$product->id,'country_id'=>$countryId,'currency'=>$product->currency,'retail_price'=>$product->retail_price,'wholesale_price'=>$product->wholesale_price,'shipping_price'=>0,'tax_rate'=>0,'active'=>1,'stock'=>$product->stock,'created_at'=>now(),'updated_at'=>now()]);}
 }
