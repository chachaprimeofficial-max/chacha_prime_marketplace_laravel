...
class VendorController extends Controller
{
 ...
 public function storeProduct(Request $request){
  $vendor=$this->vendor($request);
  $d=$request->validate(['name'=>'required|string|max:220','category_id'=>'nullable|exists:categories,id','brand_id'=>'nullable|exists:brands,id','short_description'=>'nullable|string|max:1000','description'=>'nullable|string','highlights'=>'nullable|string|max:10000','specifications'=>'nullable|string|max:15000','shipping_info'=>'nullable|string|max:5000','features'=>'nullable|string|max:10000','additional_details'=>'nullable|string|max:10000','return_policy'=>'nullable|string|max:500','gift_option'=>'nullable|boolean','fulfillment_type'=>'required|in:seller,marketplace','sold_by_type'=>'required|in:seller,marketplace_fba','video_urls'=>'nullable|string|max:10000','retail_price'=>'required|numeric|min:0','wholesale_price'=>'nullable|numeric|min:0','factory_price'=>'nullable|numeric|min:0','cost_price'=>'nullable|numeric|min:0','currency'=>'required|string|size:3','stock'=>'required|numeric|min:0','sku'=>'nullable|string|max:100','images'=>'nullable|array|max:8','images.*'=>'image|mimes:jpg,jpeg,png,webp|max:4096','videos'=>'nullable|array|max:4','videos.*'=>'file|mimes:mp4,webm,mov|max:51200']);
  $images=$request->file('images',[]);$videos=$request->file('videos',[]);unset($d['images'],$d['videos']);$d['vendor_id']=$vendor->id;$d['slug']=Str::slug($d['name']).'-'.Str::lower(Str::random(6));$d['sku']=$d['sku'] ?: 'CP-'.strtoupper(Str::random(8));$d['status']='pending';$d['stock_status']=$d['stock']>0?'in_stock':'out_of_stock';
  $product=Product::create($d);$this->saveProductImages($request,$product,$images);$this->saveProductVideos($product,$videos);app(AuditLogService::class)->log('vendor.product.created','Product',$product->id,['vendor_id'=>$vendor->id,'name'=>$product->name]);return back()->with('success','Product submitted for admin approval.');
 }

 public function updateProduct(Request $request,int $id){
  $vendor=$this->vendor($request);$p=Product::where('vendor_id',$vendor->id)->findOrFail($id);
  $d=$request->validate(['name'=>'required|string|max:220','category_id'=>'nullable|exists:categories,id','brand_id'=>'nullable|exists:brands,id','short_description'=>'nullable|string|max:1000','description'=>'nullable|string','highlights'=>'nullable|string|max:10000','specifications'=>'nullable|string|max:15000','shipping_info'=>'nullable|string|max:5000','features'=>'nullable|string|max:10000','additional_details'=>'nullable|string|max:10000','return_policy'=>'nullable|string|max:500','video_urls'=>'nullable|string|max:10000','retail_price'=>'required|numeric|min:0','wholesale_price'=>'nullable|numeric|min:0','factory_price'=>'nullable|numeric|min:0','cost_price'=>'nullable|numeric|min:0','currency'=>'required|string|size:3','stock'=>'required|numeric|min:0','sku'=>'required|string|max:100','images'=>'nullable|array|max:8','images.*'=>'image|mimes:jpg,jpeg,png,webp|max:4096','videos'=>'nullable|array|max:4','videos.*'=>'file|mimes:mp4,webm,mov|max:51200']);
  $images=$request->file('images',[]);$videos=$request->file('videos',[]);unset($d['images'],$d['videos']);$d['stock_status']=$d['stock']>0?'in_stock':'out_of_stock';$before=$p->only(['name','retail_price','stock','status']);$p->update($d);$this->saveProductImages($request,$p,$images);$this->saveProductVideos($p,$videos);app(AuditLogService::class)->log('vendor.product.updated','Product',$p->id,['before'=>$before,'after'=>$d]);return back()->with('success','Product updated.');
 }
 ...
 public function editProduct(Request $request,int $id){$vendor=$this->vendor($request);$product=Product::with(['images','videos','variants'])->where('vendor_id',$vendor->id)->findOrFail($id);$categories=DB::table('categories')->where('status',1)->orderBy('name')->get();return view('vendor.product-edit',compact('vendor','product','categories'));}
