...
class TotpService {
    ...
    public function verify(string $secret, string $code, int $window = 1): bool {
        ...
        for ($i = -$window; $i <= $window; $i++) {
            if (hash_equals($this->code($secret, $counter + $i), $code)) return true;
        }
        return false;
    }
    ...
    public function uri(string $email, string $secret): string {
        return 'otpauth://totp/Chacha%20Prime:' . rawurlencode($email)
             . '?secret=' . $secret
             . '&issuer=Chacha%20Prime&digits=6&period=30';
    }
}
