...
CREATE TABLE IF NOT EXISTS subscription_usage (
 ...
 imports_used INT UNSIGNED NOT NULL DEFAULT 0,
 products_created INT UNSIGNED NOT NULL DEFAULT 0,
 created_at TIMESTAMP NULL,
 updated_at TIMESTAMP NULL,
 UNIQUE KEY uq_subscription_usage_period (user_id,period_key),
 FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS shipments (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,order_id BIGINT UNSIGNED NOT NULL,vendor_id BIGINT UNSIGNED NOT NULL,shipping_method_id BIGINT UNSIGNED NOT NULL,tracking_number VARCHAR(150),status VARCHAR(30) DEFAULT 'pending',shipping_cost DECIMAL(18,2) DEFAULT 0,courier_user_id BIGINT UNSIGNED NULL,shipped_at TIMESTAMP NULL,delivered_at TIMESTAMP NULL,created_at TIMESTAMP NULL,updated_at TIMESTAMP NULL,FOREIGN KEY(order_id) REFERENCES orders(id),FOREIGN KEY(vendor_id) REFERENCES vendors(id),FOREIGN KEY(shipping_method_id) REFERENCES shipping_methods(id),FOREIGN KEY(courier_user_id) REFERENCES users(id) ON DELETE SET NULL,INDEX idx_shipments_courier(courier_user_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
