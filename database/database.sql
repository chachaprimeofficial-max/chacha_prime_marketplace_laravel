...
CREATE TABLE IF NOT EXISTS products (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,vendor_id BIGINT UNSIGNED NOT NULL,category_id BIGINT UNSIGNED NULL,brand_id BIGINT UNSIGNED NULL,name VARCHAR(220) NOT NULL,slug VARCHAR(240) UNIQUE NOT NULL,sku VARCHAR(100) UNIQUE NOT NULL,description LONGTEXT,short_description TEXT,retail_price DECIMAL(18,2) DEFAULT 0,cost_price DECIMAL(18,2) DEFAULT 0,currency CHAR(3) DEFAULT 'USD',stock DECIMAL(18,3) DEFAULT 0,stock_status ENUM('in_stock','out_of_stock','backorder') DEFAULT 'in_stock',status ENUM('draft','pending','published','rejected','archived') DEFAULT 'draft',featured TINYINT(1) DEFAULT 0,weight DECIMAL(12,3),created_at TIMESTAMP NULL,updated_at TIMESTAMP NULL,FOREIGN KEY(vendor_id) REFERENCES vendors(id),FOREIGN KEY(category_id) REFERENCES categories(id) ON DELETE SET NULL,FOREIGN KEY(brand_id) REFERENCES brands(id) ON DELETE SET NULL,INDEX(vendor_id,status),INDEX(category_id,status)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
